...
import { Results_view, Results_view_campo, Results_view_m2_size, Results_view_m2 } from '../entities/results_view_all';
...
export const getSizeResults = async (req: Request, res: Response) => {
    try {
        ...
        const ResultsView = await Results_view_m2_size.findBy({ id_serie: parseInt(id) })
    ...
    try {
        ...
        const save = await Promise.all(
            ResultsView.map(async (item, index) => {
                const ResultsViewM2 = await Results_view_m2.findBy({ id_result : item.id })
